...
public class ShootBehaviour : GenericBehaviour
{
	...
	private void Update()
	{
        ...
        // Handle shoot weapon action.//Input.GetMouseButtonDown(0) && !isShooting && activeWeapon > 0 && burstShotCount == 0
        if (mouseLeftDown)// && !isShooting && activeWeapon > 0 && burstShotCount == 0)
		{
            Debug.Log("Sending Bullet");
            isShooting = true;
        ...
        if (Input.GetKeyDown(KeyCode.R) && activeWeapon > 0 && isShooting == false)
		{
		...
		else if (Input.GetKeyDown(KeyCode.Q) && activeWeapon > 0)
		{
		...
		else
		{
			...
			if ((Input.GetKeyDown(KeyCode.Tab) && !isChangingWeapon))
			{
	...
	private void ShootWeapon(int weapon, bool firstShot = true)
	{
		...
		else
		{
			...
			if (Physics.Raycast(ray, out hit, 500f, shotMask))
			{
				...
				{
					// Is the target is organic?
					bool isOrganic = (organicMask == (organicMask | (1 << hit.transform.gameObject.layer)));
